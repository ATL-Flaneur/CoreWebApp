...
Install the .NET 9 SDK and *curl* with:
...
Metrics are best queried from Prometheus or Grafana.
...
If the result is a version number, check that the user is a member of the *docker* group, which is needed for running containers:
...
Troubleshooting the non-existence of the `docker` group is out of scope of this README.
...
The Dockerfile specifies running the APIs on port 8080, so map this port between the container and localhost when running the image:
...
Running Prometheus and/or Grafana daemons will prevent Docker Compose from binding to those ports. Stop the daemons using *systemctl* commands before attempting to launch with Docker Compose.
